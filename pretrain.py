                    ...
                    for topk in conf['topk']:
                        ...
                        if conf['save_results']:
                            if hit > best_hit:
                                best_hit = hit
                                np.save(
                                    result_path + 'epoch_%d_%s_hit@%d_%.4f' %
                                    (epoch, test_setting, topk, best_hit),
                                    preds.numpy())
                        if conf['save_model']:
                            if hit > best_hit:
                                best_hit = hit
...
if __name__ == '__main__':
    ...
    if os.path.exists(pretrain_model_path):
        ...
        print('use_path:', conf['path'], 'topk_u:', conf['topk_u'], 'context:',
              conf['context'], 'path_enhance:', conf['path_enhance'],
              'use_Nor:', conf['use_Nor'], 'context_enhance:',
              conf['context_enhance'], 'topk_i:', conf['topk_i'],
              "use_hard_neg:", conf['use_hard_neg'],
              "use_pretrained:", conf['use_pretrain'])
        Train_Eval(conf)
