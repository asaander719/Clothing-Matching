...
class TransR(Module):
    ...
    def __init__(self, conf, visual_features=None, item_cate=None):
        ...
        self.userEmb = F.normalize(torch.normal(
            mean=torch.zeros(self.user_num + 1, self.hidden_dim),
            std=1 / (self.hidden_dim)**0.5),
                                   p=2,
                                   dim=-1)
        self.itemEmb = F.normalize(torch.normal(
            mean=torch.zeros(self.item_num + 1, self.hidden_dim),
            std=1 / (self.hidden_dim)**0.5),
                                   p=2,
                                   dim=-1)
        ...
        self.u_embeddings_l = nn.Embedding.from_pretrained(
            self.userEmb, freeze=False, padding_idx=self.user_num)
        self.i_bias_l = nn.Embedding.from_pretrained(self.itemB,
                                                     freeze=False,
                                                     padding_idx=self.item_num)
        self.i_embeddings_i = nn.Embedding.from_pretrained(
            self.itemEmb, freeze=False, padding_idx=self.item_num)
        ...
        self.projection_matrix = nn.Embedding(
            self.user_num + 1, self.hidden_dim *
            self.hidden_dim)  #(num_relations, relation_dim * embedding_dim)
        ...
        self.visual_nn_comp = Sequential(
            Linear(conf['visual_feature_dim'], self.hidden_dim), nn.Sigmoid())

        self.visual_nn_comp[0].apply(lambda module: normal_(
            module.weight.data, mean=0, std=1 / (self.hidden_dim)**0.5))
        self.visual_nn_comp[0].apply(lambda module: normal_(
            module.bias.data, mean=0, std=1 / (self.hidden_dim)**0.5))

        self.visual_nn_per = Sequential(
            Linear(conf['visual_feature_dim'], self.hidden_dim), nn.Sigmoid())
        self.visual_nn_per[0].apply(
            lambda module: uniform_(module.weight.data, 0, 0.001))
        self.visual_nn_per[0].apply(
            lambda module: uniform_(module.bias.data, 0, 0.001))
        self.i_bias_v = nn.Embedding.from_pretrained(self.itemB,
                                                     freeze=False,
                                                     padding_idx=self.item_num)
        self.u_embeddings_v = nn.Embedding.from_pretrained(
            self.userEmb, freeze=False, padding_idx=self.user_num)
        # self.projection_matrix_v = nn.Embedding.from_pretrained(self.relationEmb, freeze=False, padding_idx=self.user_num)
        self.projection_matrix_v = nn.Embedding(
            self.user_num + 1, self.hidden_dim * self.hidden_dim)
    ...
    def forward(self, batch):
        ...
        if self.score_type == 'mlp':
            ...
            projection_matrix = self.projection_matrix(Us).view(
                Us.size(0), self.hidden_dim, self.hidden_dim).transpose(1, 2)
        ...
        if self.score_type == 'mlp':
            ...
            projection_matrix_v = self.projection_matrix_v(Us).view(
                Us.size(0), self.hidden_dim, self.hidden_dim).transpose(1, 2)
    ...
    def inference(self, batch):
        ...
        if self.score_type == 'mlp':
            ...
            projection_matrix = self.projection_matrix(Us_exp).view(
                Us_exp.size(0), Us_exp.size(1), self.hidden_dim,
                self.hidden_dim).transpose(2, 3)
            #torch.Size([bs, 2, hd*hd]) -> torch.Size([bs, 2, hd, hd])

            I_latent_ii = torch.matmul(
                I_latent_ii.unsqueeze(2), projection_matrix).squeeze(
                    2
                )  #torch.Size([1024, 2, 32]) torch.Size([1024, 2, 32, 32])
            Js_latent_ii = torch.matmul(Js_latent_ii.unsqueeze(2),
                                        projection_matrix).squeeze(2)

            J_bias_l = self.i_bias_l(J_list)
            scores = self.transE_predict(U_latent, I_latent_ii, Js_latent_ii,
                                         J_bias_l)
        ...
        if self.score_type == 'mlp':
            ...
            projection_matrix_v = self.projection_matrix_v(Us_exp).view(
                Us_exp.size(0), Us_exp.size(1), self.hidden_dim,
                self.hidden_dim).transpose(2, 3)
            #torch.Size([bs, 2, hd*hd]) -> torch.Size([bs, 2, hd, hd])

            I_visual_ii = torch.matmul(I_visual_ii.unsqueeze(2),
                                       projection_matrix_v).squeeze(2)
            Js_visual_ii = torch.matmul(Js_visual_ii.unsqueeze(2),
                                        projection_matrix_v).squeeze(2)

            J_bias_l = self.i_bias_l(J_list)
            self.vis_scores = self.transE_predict(U_visual, I_visual_ii,
