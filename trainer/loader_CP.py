...
class Train_Data_iqons(Dataset):
...
class Train_Data(Dataset):
    def __init__(self, train_data, all_bottoms):
        self.train_data = train_data
        self.cate_items = {}
        self.all_bottoms = all_bottoms
    ...
    def __getitem__(self, idx):
        u, i, j, _ = self.train_data[idx]
        candi_js = [item for item in self.all_bottoms if item != j]
        neg_j = random.choice(candi_js)
        return u, i, j, neg_j, idx
...
'''
train_data: Triplets of the form (user, given item, positive item, negative item,).
cate_items: A dictionary mapping item categories to lists of item indices.
item_cates: A dictionary mapping item indices to their respective categories.
ht_dict: A dictionary mapping entity pairs to indices in ht_paths.
ht_paths: Paths between entities in the knowledge graph.
e2re: Entity-to-relation mapping.
max_path_len: Maximum length of paths.
max_path_num: Maximum number of paths.
null_relation: A placeholder for null relations.
neg_u_path_num: Number of negative user paths.
'''
class Train_Data_Path_iqons(Dataset):
    def __init__(self,
                 train_data,
                 cate_items,
                 item_cates,
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            ij_path = [[self.null_relation] * self.max_path_len
                       ] * self.max_path_num
        ij_path_mask = torch.LongTensor(ij_path) - torch.LongTensor(
            [self.null_relation])
        ...
        try:
            ...
            ik_path = [[self.null_relation] * self.max_path_len
                       ] * self.max_path_num
        ik_path_mask = torch.LongTensor(ik_path) - torch.LongTensor(
            [self.null_relation])
        ...
        if self.neg_u_path_num != 0:
            ...
            if path != ij_path:
                ...
                mask = torch.LongTensor(path) - torch.LongTensor(
                    [self.null_relation])
            ...
            return u, i, j, neg_j, idx, torch.LongTensor(
                ij_path), torch.LongTensor(
                    ik_path), ij_path_mask, ik_path_mask, torch.LongTensor(
                        neg_u_path), torch.stack(neg_u_path_mask, dim=0)
        else:
            return u, i, j, neg_j, idx, torch.LongTensor(
                ij_path), torch.LongTensor(ik_path), ij_path_mask, ik_path_mask
...
class Test_Data_Path(Dataset):
    ...
    def __init__(self, test_data, ht_dict, ht_paths, e2re, max_path_len,
                 max_path_num, null_relation):
        self.test_data = test_data
        self.ht_dict = ht_dict
        self.ht_paths = ht_paths
        self.e2re = e2re
    ...
    def __getitem__(self, idx):
        ...
        ik_path_mask = torch.LongTensor(ik_path) - torch.LongTensor(
            [self.null_relation])
        ik_path_mask = (ik_path_mask != 0).float()
        return u, i, j, torch.LongTensor([neg_j]), torch.LongTensor(
            ij_path), torch.LongTensor(ik_path), ij_path_mask, ik_path_mask
...
class Wide_Test_Data_Path(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return u, i, j, torch.LongTensor(neg_js), torch.LongTensor(
            ij_path), torch.stack(ik_paths, dim=0), ij_path_mask, torch.stack(
                ik_path_masks, dim=0)
