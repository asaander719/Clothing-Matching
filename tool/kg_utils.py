...
def shuffle_gragh(entity2edges, entity2edge_set,
                  neighbor_samples):  #To sample edges for each entity
    ...
    for i in range(len(entity2edge_set)):
        ...
        try:
            sampled_neighbors = torch.multinomial(
                edges,
                neighbor_samples,
                replacement=len(entity2edge_set[i]) < neighbor_samples)
        except Exception:
            continue
...
def get_path(head2tails, e2re, entity2edge_set, edge2entities,
             relation2entity_set, max_path_len, path_num):
    entity_path = {}
    entity_pair_path = {}
    relation_path = {}
    ht2paths = {}
...
def get_path_iqons(head2tails, e2re, entity2edge_set, edge2entities,
             relation2entity_set, max_path_len, path_num):
    entity_path = {}
    entity_pair_path = {}
    relation_path = {}
    ...
    for cnt, (head, tails) in enumerate(head2tails):
        pdb.set_trace()
        ht2paths.update(bfs_iqons(head, tails, e2re, max_path_len))
...
def count_all_paths_iqons(inputs):
    ...
    for i, (head, tails) in enumerate(head2tails):
        ...
        ht2paths.update(bfs_iqons(head, tails, e2re, max_path_len, item_cate)) #cate+item for iqon_s dataset
    return ht2paths
...
def bfs_iqons(head, tails, e2re, max_path_len, item_cate):
    # put length-1 paths into all_paths
    # each element in all_paths is a path consisting of a sequence of (relation, entity)
    all_paths = [[i] for i in e2re[head]]
    ...
    for path in all_paths:
        ...
        if h_cate != t_cate:
            ...
            ht2paths[(head,
                      tail)].add(tuple([i[0] for i in path
                                        ]))  #only contains relation, no node
