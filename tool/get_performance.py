...
def show_multiple_cross_valid_results(performance_path,
                                      anchor,
                                      condition1=None,
                                      condition2=None):
    best_results = {}
            ...
            if condition2 is not None:
                for c in condition2:
                    if c in per_dir:
                        flag = False
            if flag:
                best_epoch, best_result = get_best_thr_cross_valid(
                    path + '/', per_dir, anchor)
                best_results[per_dir] = best_result['auc']
                best_epochs[per_dir] = best_epoch
    sorted_best_results = sorted(best_results.items(),
                                 key=lambda x: x[1],
                                 reverse=True)
...
if __name__ == '__main__':
    ...
    performance_path = '{}/performance/{}'.format(conf['root_path'], dataset)
